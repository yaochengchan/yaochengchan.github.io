---
permalink: /
title: "Product • UX Research • Human Factors"
author_profile: true
classes: wide
redirect_from: 
  - /about/
  - /about.html
---

<div style="display: flex; flex-wrap: wrap; align-items: center; gap: 24px; margin-bottom: 30px;">
  
  <div style="flex: 1 1 200px; max-width: 220px;">
    <img src="/files/homepage_me.png" alt="Profile photo" style="width: 100%; height: auto; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.08);">
  </div>

  <div style="flex: 2 1 320px;">
    <p style="font-size: 1.15em; font-weight: 600; margin-top: 0; margin-bottom: 8px; color: #222;">
      Connecting user insights, system requirements, and data-driven product decisions.
    </p>
    <p style="margin: 0 0 10px 0; line-height: 1.6;">
      I specialize in evaluating human-system interactions across physical and digital environments. My work combines qualitative and quantitative user research, cognitive ergonomics (eye-tracking), and rapid prototype validation to solve critical challenges around usability, intent legibility, and user trust.
    </p>
    <p style="margin: 0; font-size: 0.95em; color: #555;">
      Experienced in navigating projects from early problem discovery and MVP scoping to experimental validation, cross-functional alignment, and product design guidelines.
    </p>
  </div>

</div>

## How My Background Maps to Industry Roles

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 16px; margin-bottom: 35px;">
  
  <div style="border: 1px solid #e1e4e8; border-radius: 8px; padding: 16px; background: #fafbfc;">
    <div style="font-weight: 700; color: #0366d6; margin-bottom: 6px;">Mixed-Methods & Quantitative UXR</div>
    <p style="margin: 0; font-size: 0.9em; line-height: 1.45; color: #333;">
      End-to-end qualitative research (semi-structured interviews, thematic coding, grounded theory, in-situ observation) paired with rigorous experimental design, survey validation, and advanced statistical modeling (LMM, MANOVA) to translate behavioral signals into design roadmaps.
    </p>
  </div>

  <div style="border: 1px solid #e1e4e8; border-radius: 8px; padding: 16px; background: #fafbfc;">
    <div style="font-weight: 700; color: #0366d6; margin-bottom: 6px;">Human Factors & Systems Safety</div>
    <p style="margin: 0; font-size: 0.9em; line-height: 1.45; color: #333;">
      Evaluating cognitive workload, situational awareness, attention allocation (eye-tracking/pupillometry), and proxemics in safety-critical, shared human-machine environments.
    </p>
  </div>

  <div style="border: 1px solid #e1e4e8; border-radius: 6px; padding: 16px; background: #fafbfc;">
    <div style="font-weight: 700; color: #0366d6; margin-bottom: 6px;">Product Management</div>
    <p style="margin: 0; font-size: 0.9em; line-height: 1.45; color: #333;">
      Translating ambiguous user problems into clear product requirements and roadmaps. Experienced in scoping MVPs, prioritizing feature trade-offs, aligning cross-functional teams, and leveraging user data to guide product strategy.
    </p>
  </div>

</div>

## Core Toolkit

<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px; margin-bottom: 35px;">
  <div style="border: 1px solid #e1e4e8; border-radius: 8px; padding: 14px; background: #ffffff;">
    <strong>Product & Strategy</strong>
    <p style="margin: 6px 0 0 0; font-size: 0.88em; line-height: 1.4; color: #444;">Product Requirements (PRDs), MVP Scoping, Feature Prioritization, User Journey Mapping, Cross-Functional Alignment, Usability Benchmarking</p>
  </div>
  <div style="border: 1px solid #e1e4e8; border-radius: 8px; padding: 14px; background: #ffffff;">
    <strong>User Research & Testing</strong>
    <p style="margin: 6px 0 0 0; font-size: 0.88em; line-height: 1.4; color: #444;">Semi-Structured Interviews, Thematic Analysis, Controlled Lab Studies, Eye-Tracking, Field Evaluations, Survey Design (Qualtrics)</p>
  </div>
  <div style="border: 1px solid #e1e4e8; border-radius: 8px; padding: 14px; background: #ffffff;">
    <strong>Data & Analytics</strong>
    <p style="margin: 6px 0 0 0; font-size: 0.88em; line-height: 1.4; color: #444;">Linear Mixed Models (LMM), MANOVA/ANOVA, Non-parametric Tests, Hypothesis Testing, SPSS, R, Python</p>
  </div>
  <div style="border: 1px solid #e1e4e8; border-radius: 8px; padding: 14px; background: #ffffff;">
    <strong>Prototyping & Tools</strong>
    <p style="margin: 6px 0 0 0; font-size: 0.88em; line-height: 1.4; color: #444;">Figma, CAD & 3D Printing, Python, Raspberry Pi Prototyping, Git, ROS</p>
  </div>
</div>

## Selected Work & Case Studies

<div style="display: flex; flex-direction: column; gap: 24px; margin-top: 20px;">

  <!-- Card 1: Deployment & Human Reaction -->
  <div class="portfolio-card">
    <div style="flex: 1 1 260px; max-width: 320px; margin-right: 24px; margin-bottom: 12px;">
      <img src="/files/phase2_fail.PNG" alt="Deployment and Human Reaction" class="card-thumbnail">
    </div>
    <div style="flex: 1 1 320px;">
      <div style="margin-bottom: 8px;">
        <span class="badge-tag">Human Factors</span>
        <span class="badge-tag">Field Deployment</span>
        <span class="badge-tag">Pedestrian Safety</span>
      </div>
      <h3 style="margin: 0 0 8px 0;">Robot Deployment & Human Reaction</h3>
      <p style="margin: 0 0 14px 0; font-size: 0.93em; line-height: 1.5; color: #333;">Conducted multi-phase field studies evaluating expressive signaling modalities (gaze cues, light patterns, directional indicators) on autonomous mobile platforms in busy pedestrian zones. Analyzed real-world attentional limits, proxemics, and human compliance to establish safety guidelines.</p>
      <a href="/HRE/" class="btn-case-study">View Deployment Case Study →</a>
    </div>
  </div>

  <!-- Card 2: Hardware Prototyping -->
  <div class="portfolio-card">
    <div style="flex: 1 1 260px; max-width: 320px; margin-right: 24px; margin-bottom: 12px;">
      <img src="/files/prototype_head.gif" alt="Hardware Prototyping" class="card-thumbnail">
    </div>
    <div style="flex: 1 1 320px;">
      <div style="margin-bottom: 8px;">
        <span class="badge-tag">Product Management</span>
        <span class="badge-tag">Hardware MVP</span>
        <span class="badge-tag">Rapid Testbed</span>
      </div>
      <h3 style="margin: 0 0 8px 0;">Hardware Prototyping: Behavioral Testbed</h3>
      <p style="margin: 0 0 14px 0; font-size: 0.93em; line-height: 1.5; color: #333;">Designed and built an MVP 2-DOF motor-mirroring puppeteering interface using Raspberry Pi microcomputers, Dynamixel servos, and custom CAD enclosures. Created a real-time physical testbed to evaluate user perception and responsive signaling without waiting for full autonomous software pipelines.</p>
      <a href="/puppet/" class="btn-case-study">View Prototyping Case Study →</a>
    </div>
  </div>

  <!-- Card 3: Method Validation -->
  <div class="portfolio-card">
    <div style="flex: 1 1 260px; max-width: 320px; margin-right: 24px; margin-bottom: 12px;">
      <img src="/files/video2.png" alt="Method Validation" class="card-thumbnail">
    </div>
    <div style="flex: 1 1 320px;">
      <div style="margin-bottom: 8px;">
        <span class="badge-tag">UX Methodology</span>
        <span class="badge-tag">Experimental Rigor</span>
        <span class="badge-tag">Validity Testing</span>
      </div>
      <h3 style="margin: 0 0 8px 0;">Method Validation: Simulation vs. Field Testing</h3>
      <p style="margin: 0 0 14px 0; font-size: 0.93em; line-height: 1.5; color: #333;">Formulated an experimental benchmarking framework comparing remote video-based evaluations against in-person Wizard-of-Oz field deployments. Quantified where low-cost digital testing reliably predicts user behavior and where physical in-situ testing is required.</p>
      <a href="/vid2real/" class="btn-case-study">View Validation Case Study →</a>
    </div>
  </div>

  <!-- Card 4: Interaction Design -->
  <div class="portfolio-card">
    <div style="flex: 1 1 260px; max-width: 320px; margin-right: 24px; margin-bottom: 12px;">
      <img src="/files/pepper.png" alt="Interaction Design" class="card-thumbnail">
    </div>
    <div style="flex: 1 1 320px;">
      <div style="margin-bottom: 8px;">
        <span class="badge-tag">Interaction Design</span>
        <span class="badge-tag">Behavioral Testing</span>
        <span class="badge-tag">Product Adoption</span>
      </div>
      <h3 style="margin: 0 0 8px 0;">Multimodal Interaction Design for Service Interfaces</h3>
      <p style="margin: 0 0 14px 0; font-size: 0.93em; line-height: 1.5; color: #333;">Designed multimodal communicative profiles (gestures, verbal phrasing, non-verbal cues) across distinct user archetypes. Measured the resulting impact on task completion, user satisfaction, and adoption intent across customer service scenarios.</p>
      <a href="/personality/" class="btn-case-study">View Interaction Case Study →</a>
    </div>
  </div>

</div>
